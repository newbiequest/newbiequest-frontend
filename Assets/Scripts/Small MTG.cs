...
public class SmallMTG : MonoBehaviour
{
    [Header("Trigger")]
    ...
    private const int MinHeadcount = 2;
    private const int MaxHeadcount = 7;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        areYouStart.OpenPanel("SMALL_MTG");
    }
    ...
    private string GetComment(int score, bool countCorrect, bool startCorrect, bool purposeFilled)
    {
        if (countCorrect && startCorrect && purposeFilled)
            return score >= 95 ? "완벽해요. 소회의실 예약 조건을 모두 정확히 맞췄습니다." : "예약 정보는 정확합니다. 다음에는 조금 더 빠르게 처리해보세요.";
